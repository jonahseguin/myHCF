 ...
 *         Shawckz.com
 */
public class GCmdArgs {

    private final GCommandSender sender;
    private final String[] args;

    public GCmdArgs(GCommandSender sender, String[] args) {
        this.sender = sender;
        this.args = args;
    }

    public GCommandSender getSender() {
        return sender;
    }
    ...
    public String[] getTrimmedArgs(int start, int end) {
        if (args.length < start || args.length < end) {
            throw new GCommandException("Args not long enough to trim");
        }
