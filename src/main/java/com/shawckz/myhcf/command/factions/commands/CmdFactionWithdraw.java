...
@FCommand(name = "withdraw", desc = "Withdraw money from your faction", usage = "/f withdraw <amount>", minArgs = 1, playerOnly = true)
public class CmdFactionWithdraw implements HCFCommand {
    ...
    public void onCommand(FCmdArgs cmdArgs) {
        ...
        if(player.getFaction() != null) {
            ...
            if(player.getFaction().getBalance() >= amount) {
                player.getFaction().setBalance(player.getFaction().getBalance() - amount);
                player.setBalance(player.getBalance() + amount);
                player.getFaction().sendMessage(FLang.format(FactionLang.FACTION_WITHDRAW, p.getName(), amount+""));
            }
            else{
                FLang.send(p, FactionLang.FACTION_WITHDRAW_INSUFFICIENT_FUNDS);
            }
        }
        else{
