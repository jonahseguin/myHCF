...
@FCommand(name = "setrally", desc = "Set your faction's rally", usage = "/f setrally", playerOnly = true, aliases = {"setr"})
public class CmdFactionSetRally implements HCFCommand {
    ...
    public void onCommand(FCmdArgs cmdArgs) {
        ...
        if(player.getFaction() != null) {
            if(player.getFactionRole() != FactionRole.MEMBER) {
                player.getFaction().setRally(p.getLocation());
                player.getFaction().sendMessage(FLang.format(FactionLang.FACTION_SETRALLY_LOCAL, player.getName()));
            }
            else{
